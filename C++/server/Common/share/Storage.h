...
public:
	 ...
	 */
	SUNYOU_INLINE T * Get() { return Pointer; }

	/** Sets the current object to P
	 */
	SUNYOU_INLINE void Set(T * P) { Pointer = P; }

	/** Are we at the end of the storage container?
	 */
	SUNYOU_INLINE bool AtEnd() { return (Pointer == 0); }
...
public:
	...
	SUNYOU_INLINE void SetEntry(uint32 Entry, T* p){ _storage.SetEntry(Entry, p); }
	SUNYOU_INLINE char * GetIndexName() { return _indexName; }
	SUNYOU_INLINE char * GetFormatString() { return _formatString; }
	...
	uint32 Size(){ return _storage.Size(); }
	...
	{
		MyLog::log->notice("Deleting database cache of `%s`...\n", _indexName);
...
public:
	 ...
	 */
	SUNYOU_INLINE void LoadBlock(Field * fields, T * Allocated)
	{
	...
	void Load(const char * IndexName, const char * FormatString)
	{
		...
		if(Storage<T, StorageType>::_storage.NeedsMax())
		{
			...
			if(result)
			{
				...
				{
					MyLog::log->error("The table, '%s', has a maximum entry of %u, which is less %u. Any items higher than %u will not be loaded.",
		...
		if(result->GetFieldCount() != cols)
		{
			if(result->GetFieldCount() > cols)
			{
				MyLog::log->warn("Invalid format in %s (%u/%u), loading anyway because we have enough data\n", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
			}
			else
			{
				MyLog::log->warn("Invalid format in %s (%u/%u), not enough data to proceed.\n", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
				delete result;
				return;
			}
		}
		...
		MyLog::log->notice("%u entries loaded from table %s.", result->GetRowCount(), IndexName);
		delete result;
	...
	void LoadAdditionalData(const char * IndexName, const char * FormatString)
	{
		...
		if(result->GetFieldCount() != cols)
		{
			...
			{
				MyLog::log->error("Invalid format in %s (%u/%u), not enough data to proceed.\n", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
				delete result;
				return;
			}
		}
	...
	void Reload()
	{
		MyLog::log->notice("Reloading database cache from `%s`...\n", Storage<T, StorageType>::_indexName);
		...
		if(result->GetFieldCount() < cols)
		{
			MyLog::log->error("Invalid format in %s (%u/%u).", Storage<T, StorageType>::_indexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
			delete result;
			return;
		}
