...
bool ConfigFile::SetSource(const char *file, bool ignorecase)
{
	...
	if(file != 0)
	{
		...
		if(!f)
		{
			MyLog::log->error("Could not open %s.", file);
			return false;
		}
...
parse:
			...
			if(in_block)
			{
				...
				if(in_multiline_quote)
				{
					...
					{
						MyLog::log->error("Quote without variable.");
						return false;
					}
			...
			else
			{
				...
				if(offset != string::npos)
				{
					...
					{
						MyLog::log->error("Block without name.");
						return false;
					}
			...
			{
				MyLog::log->error("Exception in config parsing.");
				return false;
			}
		...
		if(in_block)
		{
			MyLog::log->error("Unterminated block.");
			return false;
		}

		if(in_multiline_comment)
		{
			MyLog::log->error("Unterminated comment.");
			return false;
		}

		if(in_multiline_quote)
		{
			MyLog::log->error("Unterminated quote.");
			return false;
		}
