...
bool CThreadPool::ThreadExit(Thread * t)
{
	...
	MyLog::log->debug("ThreadPool : Thread %u entered the free pool.", t->ControlInterface.GetId());
	_mutex.Release();
	return true;
}
...
void CThreadPool::ExecuteTask(ThreadBase * ExecutionTarget)
{
	...
	if(m_freeThreads.size())
	{
		...
		MyLog::log->debug("ThreadPool : Thread %u left the thread pool.", t->ControlInterface.GetId());
	}
	else
	{
	...
	MyLog::log->debug("ThreadPool : Thread %u is now executing task at %p.", t->ControlInterface.GetId(), ExecutionTarget);
...
void CThreadPool::Startup()
{
	...
	MyLog::log->debug("ThreadPool : Startup, launched %u threads.", tcount);
}

void CThreadPool::ShowStats()
{
	_mutex.Acquire();
	MyLog::log->debug("ThreadPool : ============ ThreadPool Status =============");
	MyLog::log->debug("ThreadPool : Active Threads: %u", m_activeThreads.size());
	MyLog::log->debug("ThreadPool : Suspended Threads: %u", m_freeThreads.size());
	MyLog::log->debug("ThreadPool : Requested-To-Freed Ratio: %.3f%% (%u/%u)", float( float(_threadsRequestedSinceLastCheck+1) / float(_threadsExitedSinceLastCheck+1) * 100.0f ), _threadsRequestedSinceLastCheck, _threadsExitedSinceLastCheck);
	MyLog::log->debug("ThreadPool : Eaten Count: %d (negative is bad!)", _threadsEaten);
	MyLog::log->debug("ThreadPool : ============================================");
	_mutex.Release();
}
...
void CThreadPool::IntegrityCheck()
{
    ...
    if(gobbled < 0)
	{
		...
		MyLog::log->debug("ThreadPool : IntegrityCheck: (gobbled < 0) Spawning %u threads.", new_threads);
	}
	else if(gobbled < THREAD_RESERVE)
	{
		...
		MyLog::log->debug("ThreadPool : IntegrityCheck: (gobbled <= 5) Spawning %u threads.", new_threads);
	}
	else if(gobbled > THREAD_RESERVE)
	{
		...
		MyLog::log->debug("ThreadPool : IntegrityCheck: (gobbled > 5) Killing %u threads.", kill_count);
	}
	else
	{
		// perfect! we have the ideal number of free threads.
		MyLog::log->debug("ThreadPool : IntegrityCheck: Perfect!");
	}
...
{
	MyLog::log->debug("ThreadPool : Killing %u excess threads.", count);
	_mutex.Acquire();
	Thread * t;
...
void CThreadPool::Shutdown()
{
	...
	MyLog::log->debug("ThreadPool : Shutting down %u threads.", tcount);
	...
	for(;;)
	{
		...
		{
			MyLog::log->debug("ThreadPool : %u threads remaining...",m_activeThreads.size() + m_freeThreads.size() );
			_mutex.Release();
			Sleep(1000);
			continue;
		}
...
static unsigned long WINAPI thread_proc(void* param)
{
	...
	for(;;)
	{
		...
		{
			MyLog::log->debug("ThreadPool : Thread %u exiting.", tid);
			break;
		}
		else
		{
			if(ht)
				MyLog::log->debug("ThreadPool : Thread %u waiting for a new task.", tid);
...
static void * thread_proc(void * param)
{
	Thread * t = (Thread*)param;
	t->SetupMutex.Acquire();
	MyLog::log->info( "Thread %u started.", t->ControlInterface.GetId() );
	t->SetupMutex.Release();
