...
protected:
	HANDLE create_anonymous_semaphore(long cur,long max)
	{
		...
		SUNYOU_INLINE void shift_generations_down()
		{
		...
		SUNYOU_INLINE void dispose_entry(list_entry& entry)
		{
		...
		SUNYOU_INLINE HANDLE duplicate_handle(HANDLE source)
		{
...
{
public:
	SUNYOU_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	SUNYOU_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	SUNYOU_INLINE void EndSynchronized()
	{
...
{
public:
	SUNYOU_INLINE Condition(Mutex *m)
	{
		mut=m;
		pthread_cond_init(&cond,NULL);
	}
	SUNYOU_INLINE ~Condition()
	{
		pthread_cond_destroy(&cond);
	}

	SUNYOU_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	SUNYOU_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	SUNYOU_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	SUNYOU_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
