...
public:
	...
	SUNYOU_INLINE uint32 GetId() { return (uint32)thread_id; }
};
...
public:
	...
	SUNYOU_INLINE void Gobble() { _threadsEaten=(int32)m_freeThreads.size(); }

	// gets active thread count
	SUNYOU_INLINE uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size(); }

	// gets free thread count
	SUNYOU_INLINE uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};
