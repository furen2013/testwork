...
void CGameServer::_UnhookSignals()
{
	signal(SIGINT, 0);
	signal(SIGTERM, 0);
	signal(SIGABRT, 0);
...
void CGameServer::_OnSignal(int s)
{
	switch (s)
	{
	case SIGINT:
...
#ifdef WIN32
	{
		...
		if(Aff > 0)
		{
			...
			if(GetProcessAffinityMask(hProcess,&appAff,&sysAff))
			{
				...
				if(!curAff )
				{
					MyLog::log->error("Processors marked in UseProcessors bitmask (hex) %x not accessible for phonegs. Accessible processors bitmask (hex): %x",Aff,appAff);
				}
				else
				{
		...
		if(Prio)
		{
			if(SetPriorityClass(hProcess,HIGH_PRIORITY_CLASS))
				MyLog::log->info("phonegs process priority class set to HIGH");
			else
				MyLog::log->error("ERROR: Can't set phone gs process priority class.");
			MyLog::log->info("");
		}
	}
#endif
