...
bool same_month( unsigned int t1, unsigned int t2 )
{
	...
	time_t _t2 = t2;
	tm* p2 = localtime( &_t2 );
	return ( p1->tm_year == p2->tm_year && p1->tm_mon == p2->tm_mon );
}
...
bool same_week( unsigned int t1, unsigned int t2 )
{
	...
	if( p1->tm_year == p2->tm_year && p1->tm_mon == p2->tm_mon )
		if( p1->tm_wday + ( p2->tm_mday - p1->tm_mday ) < 7 )
			return true;
	return false;
}
...
{
	unsigned int checksum = ssl::__crc32( (const unsigned char*)data, length );
	//printf( "send_index = %d\n", send_index );
...
{
	unsigned int tmp = ssl::__crc32( (const unsigned char*)data, length );
	//printf( "recv_index = %d\n", recv_index );
